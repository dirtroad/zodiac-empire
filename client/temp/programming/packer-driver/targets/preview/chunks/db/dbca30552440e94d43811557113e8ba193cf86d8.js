    ...
    execute: function () {
      ...
      _export("GameManager", GameManager = (_dec = ccclass('GameManager'), _dec2 = property(Node), _dec(_class = (_class2 = (_class3 = class GameManager extends Component {
        ...
        loadGameData() {
          ...
          return _asyncToGenerator(function* () {
            try {
              var _this2$_userData;

              _this2._userData = yield (_crd && api === void 0 ? (_reportPossibleCrUseOfapi({
                error: Error()
              }), api) : api).getUserInfo();
              _this2._emotionData = yield (_crd && api === void 0 ? (_reportPossibleCrUseOfapi({
                error: Error()
              }), api) : api).getEmotionResources();
              console.log('📊 用户数据:', (_this2$_userData = _this2._userData) == null ? void 0 : _this2$_userData.nickname);
              console.log('💫 情绪资源:', _this2._emotionData.length, '种');
            } catch (e) {
        ...
        switchPage(page) {
          ...
          var currentPageNode = this._pages.get(this._currentPage);
          ...
          var newPageNode = this._pages.get(page);
