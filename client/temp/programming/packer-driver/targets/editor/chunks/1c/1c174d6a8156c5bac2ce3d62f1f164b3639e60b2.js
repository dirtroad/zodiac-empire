  ...
  return {
    ...
    execute: function () {
      ...
      _export("TestScene", TestScene = (_dec = ccclass('TestScene'), _dec(_class = class TestScene extends Component {
        ...
        async loginAndLoad() {
          try {
            const loginRes = await this.api('/auth/wechat/login', 'POST', {
              code: 'test'
            });

            if (loginRes.code === 0) {
              this.token = loginRes.data.access_token;
              this.userData = loginRes.data.user; // 保存到本地存储
        ...
        async loadEmotionData() {
          try {
            ...
            if (res.code === 0) {
              this.emotionData = res.data.map(e => ({
        ...
        async submitEmotionPost(emotionType, emotionName, icon) {
          if (!this.token) {
            console.log('请先登录');
            return;
          }
        ...
        async doGacha() {
          ...
          try {
            const res = await this.api('/equipment/gacha', 'POST', {
              count: 1
            });
            ...
            if (res.code === 0 && res.data) {
              ...
              this.showGachaResult(res.data); // 刷新用户数据
        ...
        async showTimeBank() {
          ...
          this.addLabel('⏰ 时空银行', 0, 240, 36, new Color(100, 200, 255)); // 返回按钮
          ...
          this.addLabel(`${((_this$userData4 = this.userData) == null ? void 0 : _this$userData4.bankedTimeCoin) || 0}`, 0, 130, 42, new Color(100, 255, 200)); // 存取按钮
          ...
          this.addLabel('存入1000时空晶体，每日获得1个利息', 0, -130, 14, new Color(100, 100, 100)); // 挂机收益
        ...
        showWithdrawUI() {
          ...
          this.addLabel(`银行余额: ${((_this$userData5 = this.userData) == null ? void 0 : _this$userData5.bankedTimeCoin) || 0}`, 0, 150, 24, new Color(100, 200, 255)); // 输入金额
        ...
        async depositTimeCoin(amount) {
          ...
          try {
            const res = await this.api('/users/bank/deposit', 'POST', {
              amount
            });
        ...
        async withdrawTimeCoin(amount) {
          ...
          try {
            const res = await this.api('/users/bank/withdraw', 'POST', {
              amount
            });
        ...
        createTabBar() {
          ...
          tabs.forEach((t, i) => {
            ...
            const color = active ? new Color(255, 215, 0) : new Color(150, 150, 150); // 图标
